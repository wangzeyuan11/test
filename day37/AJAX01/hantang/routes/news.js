...
router.get('/list', (req, res, next) => {
	...
	//执行sql命令
	connection.query(
		'select nid,title,ctime from ht_news where cat_id=? order by ctime desc limit ?,?;select count(*) as n from ht_news where cat_id=?',
		[obj.cat_id, start, size, obj.cat_id], (err, r) => {
			if (err) {
				return next(err)
			}
			console.log(r)
