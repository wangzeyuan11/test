...
$(function(){
    ...
    $.get(url,data=>{
        ...
        //习惯：jQuery类型的对象，用$符做开头，达到见名知意的效果
        const $btn_next = $('.menu>.pages>.next')

        page == pageCount ? $btn_next.hide() : $btn_next.show()

        //浏览器的缓存问题:默认属于强缓存机制，会自动存储所有下载的图片，
        //短时间内重复的请求 都会直接使用本地的图片，实现节省流量，提升速度

        //统一体验：当新的页面内容出现时，直接滚动回到顶部
        //参数代表 距离顶部的偏移量. 0代表没有偏移
        $(window).scrollTop(0)
    })
 }
    ...
    $('.menu>.pages>.next').click(function(){
        //获取当前激活页上的数字.利用*1把文本转数字，避免后续的 字符串拼接问题
